...
public class InstructionMap {
    ...
    public static final InstructionMap DEFAULT = new InstructionMap(CodeTable.Default,
            AddressCache.defaultLastMode());

    public InstructionMap(CodeTable codeTable, short maxMode) {
        this.singleInstructionMap = new SingleInstructionMap(
                Instruction.TYPE_LAST_INSTRUCTION + maxMode + 1,
                findInstMaxSize(codeTable.entries, 0));
        this.combinedInstructionMap = new CombinedInstructionMap(
                Instruction.TYPE_LAST_INSTRUCTION + maxMode + 1,
    ...
    private static class SingleInstructionMap {
        ...
        short lookup(Instruction inst) {
            int instMode = (inst.getIst() == Instruction.TYPE_COPY) ?
                    (inst.getIst() + inst.getMode()) : inst.getIst();
